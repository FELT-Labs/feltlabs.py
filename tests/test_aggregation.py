...
def test_analytics_mean_aggregation():
    ...
    assert np.allclose(model1.predict(X), mean(y[0]))
    assert np.allclose(model2.predict(X), mean(y[1]))

    agg_model = model1.new_model()
    agg_model.aggregate([model2])
    assert np.allclose(agg_model.predict(X), mean(y[0] + y[1]))

    agg_model = model1.new_model()
    agg_model.aggregate([model3])
    assert np.allclose(agg_model.predict(X), mean(y[0] + y[2]))

    enc_model1 = model1.new_model()
    enc_model1.add_noise(10)
    assert not np.allclose(enc_model1.predict(X), mean(y[0]))

    enc_model2 = model2.new_model()
    enc_model2.add_noise(11)
    assert not np.allclose(enc_model2.predict(X), mean(y[1]))

    enc_model3 = model3.new_model()
    enc_model3.add_noise(12)
    assert not np.allclose(enc_model3.predict(X), mean(y[2]))

    final_model = enc_model1
    final_model.aggregate([enc_model2, enc_model3])
    final_model.remove_noise_models([10, 11, 12])
    assert np.allclose(final_model.predict(X), mean(y[0] + y[1] + y[2]))
